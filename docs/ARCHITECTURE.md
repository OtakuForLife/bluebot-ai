...
This document describes the concrete technologies and tooling that will be used to implement the requirements in [[REQUIREMENTS]], while respecting the constraints of being **offline-first** and **100% open source**.
...
These technology choices directly support the functional and non-functional requirements in [[REQUIREMENTS]] while leaving room for future enhancements such as plugin systems, visual workflow editors, and version-control-aware workflows.
